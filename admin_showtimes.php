...
// Đếm tổng số bản ghi
$sql_count = "SELECT COUNT(*) FROM showtimes s JOIN movies m ON s.movie_id = m.id JOIN cinemas c ON s.cinema_id = c.id WHERE $where_sql";
...
function getUrl($p, $search, $cinema, $date) {
    $params = ['page' => $p];
    if($search) $params['search'] = $search;
    if($cinema) $params['cinema_id'] = $cinema;
    ...
    <main class="flex-1 flex flex-col overflow-hidden">
        ...
        <div class="flex-1 overflow-y-auto p-8 custom-scrollbar">
            ...
            <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8">
                ...
                <a href="add_showtime.php" class="bg-primary hover:bg-primary/90 text-background-dark px-6 py-3 rounded-xl font-bold flex items-center gap-2 shadow-[0_4px_14px_0_rgba(242,204,13,0.39)] transition-all">
                    <span class="material-symbols-outlined">add</span> Tạo lịch chiếu
                </a>
            </div>
            ...
            <form method="GET" action="admin_showtimes.php" class="bg-surface-dark border border-accent-dark p-4 rounded-2xl mb-8 flex flex-col lg:flex-row gap-4 shadow-lg">
                <div class="flex-1 relative">
                    ...
                    <input type="text" name="search" value="<?php echo htmlspecialchars($search_query); ?>" autocomplete="off" placeholder="Tìm tên phim, phòng chiếu..." class="w-full bg-accent-dark/30 border border-accent-dark rounded-xl py-3 pl-10 pr-4 text-slate-200 outline-none focus:border-primary">
                </div>
                <div class="flex flex-wrap gap-4">
                    <select name="cinema_id" class="bg-accent-dark/30 border border-accent-dark rounded-xl py-3 px-4 text-slate-200 outline-none focus:border-primary min-w-[180px]">
                        <option class="bg-surface-dark text-slate-100" value="">Tất cả rạp</option>
                        <?php foreach($cinemas as $c): ?>
                            <option class="bg-surface-dark text-slate-100" value="<?php echo $c['id']; ?>" <?php echo $filter_cinema == $c['id'] ? 'selected' : ''; ?>><?php echo htmlspecialchars($c['name']); ?></option>
                        <?php endforeach; ?>
                    </select>
                    <input type="date" name="date" value="<?php echo htmlspecialchars($filter_date); ?>" class="bg-accent-dark/30 border border-accent-dark rounded-xl py-3 px-4 text-slate-200 outline-none focus:border-primary min-w-[180px]" style="color-scheme: dark;">
            ...
            <div class="bg-surface-dark border border-accent-dark rounded-2xl overflow-hidden shadow-2xl flex flex-col">
                    ...
                    <table class="w-full text-left border-collapse min-w-[1000px]">
                        ...
                        <tbody class="divide-y divide-accent-dark/50">
                            <?php foreach ($showtimes as $st): 
                                ...
                                $is_full = ($booked_seats >= 100);
                                $percent = ($booked_seats / 100) * 100;
                                $is_past = (strtotime($st['show_date'] . ' ' . $st['start_time']) < time());
                                
                                $row_classes = "hover:bg-accent-dark/20 transition-colors group";
                                if ($is_past) $row_classes .= " opacity-50 grayscale hover:opacity-100 hover:grayscale-0";
                                elseif ($is_full) $row_classes .= " bg-red-500/5";
                            ?>
                            <tr class="<?php echo $row_classes; ?>">
                                <td class="px-6 py-4">
                                    <div class="flex items-center gap-3">
                                        <div class="h-14 w-10 bg-slate-800 rounded overflow-hidden flex-shrink-0 border border-accent-dark"><img class="h-full w-full object-cover" src="<?php echo htmlspecialchars($st['poster_url']); ?>" alt=""/></div>
                                        <span class="text-slate-100 font-bold group-hover:text-primary transition-colors max-w-[180px] truncate"><?php echo htmlspecialchars($st['movie_title']); ?></span>
                                    </div>
                                </td>
                                <td class="px-6 py-4 text-slate-300 text-sm"><?php echo htmlspecialchars($st['cinema_name']); ?></td>
                                <td class="px-6 py-4 text-center"><span class="px-3 py-1 bg-background-dark/50 border border-accent-dark rounded-lg font-mono text-sm text-slate-300"><?php echo htmlspecialchars($st['room_name']); ?></span></td>
                                <td class="px-6 py-4 text-center">
                                    <div class="flex flex-col items-center">
                                        <span class="text-slate-100 font-bold text-sm"><?php echo date('H:i', strtotime($st['start_time'])); ?></span>
                                        <span class="text-primary text-[10px] uppercase tracking-widest mt-1"><?php echo date('d/m/Y', strtotime($st['show_date'])); ?></span>
                                    </div>
                                </td>
                                <td class="px-6 py-4 text-center">
                                    <div class="flex flex-col items-center gap-1.5">
                                        <span class="text-xs font-bold tracking-wider <?php echo $is_past ? 'text-slate-400' : ($is_full ? 'text-red-500' : ($booked_seats > 0 ? 'text-primary' : 'text-slate-400')); ?>"><?php echo $booked_seats; ?> / 100</span>
                                        <div class="w-20 h-1.5 bg-background-dark rounded-full overflow-hidden border border-accent-dark/50"><div class="h-full <?php echo $is_past ? 'bg-slate-500' : ($is_full ? 'bg-red-500' : 'bg-primary'); ?>" style="width: <?php echo $percent; ?>%"></div></div>
                                    </div>
                                </td>
                                <td class="px-6 py-4 text-center">
                                    <?php if ($is_past): ?><span class="px-3 py-1 rounded-full text-[10px] font-bold uppercase bg-slate-500/10 text-slate-400 border border-slate-500/20">Đã chiếu</span>
                                    <?php elseif ($is_full): ?><span class="px-3 py-1 rounded-full text-[10px] font-bold uppercase bg-red-500/10 text-red-500 border border-red-500/20">Hết vé</span>
                                    <?php else: ?><span class="px-3 py-1 rounded-full text-[10px] font-bold uppercase bg-green-500/10 text-green-500 border border-green-500/20">Đang chiếu</span>
                                    <?php endif; ?>
                                </td>
                                <td class="px-6 py-4 text-right whitespace-nowrap">
                                    <a href="admin_showtimes.php?action=delete&id=<?php echo $st['id']; ?>" onclick="return confirm('Xóa lịch chiếu này?');" class="inline-block p-2 rounded-lg text-slate-400 hover:bg-red-500/10 hover:text-red-500 transition-all"><span class="material-symbols-outlined text-sm">delete</span></a>
                                </td>
                            </tr>
                            <?php endforeach; ?>
                            <?php if(count($showtimes) == 0): ?>
                            <tr><td colspan="7" class="px-6 py-12 text-center text-slate-500">Chưa có lịch chiếu nào.</td></tr>
                            <?php endif; ?>
                        </tbody>
                    </table>
                </div>
                ...
                <div class="px-6 py-4 border-t border-accent-dark bg-background-dark/30 flex flex-col md:flex-row items-center justify-between gap-4">
                    <p class="text-sm text-slate-500 font-medium">Trang <span class="text-slate-100"><?php echo $page; ?></span> / <?php echo $total_pages; ?> (<?php echo $total_records; ?> suất)</p>
                    <div class="flex items-center gap-1">
                        <a href="<?php echo getUrl(1, $search_query, $filter_cinema, $filter_date); ?>" class="px-2 py-1.5 rounded-lg bg-surface-dark border border-accent-dark text-slate-400 hover:text-primary transition-all <?php if($page <= 1) echo 'pointer-events-none opacity-30'; ?>" title="Trang đầu"><span class="material-symbols-outlined text-sm">first_page</span></a>
                        <a href="<?php echo getUrl(max(1, $page - 1), $search_query, $filter_cinema, $filter_date); ?>" class="px-2 py-1.5 rounded-lg bg-surface-dark border border-accent-dark text-slate-400 hover:text-primary transition-all <?php if($page <= 1) echo 'pointer-events-none opacity-30'; ?>"><span class="material-symbols-outlined text-sm">chevron_left</span></a>
                        ...
                        for ($i = $start; $i <= $end; $i++) {
                            ...
                            echo '<a href="'.getUrl($i, $search_query, $filter_cinema, $filter_date).'" class="px-3 py-1.5 text-sm font-bold rounded-lg border border-accent-dark transition-all '.$active.'">'.$i.'</a>';
                        }
                        ?>

                        <a href="<?php echo getUrl(min($total_pages, $page + 1), $search_query, $filter_cinema, $filter_date); ?>" class="px-2 py-1.5 rounded-lg bg-surface-dark border border-accent-dark text-slate-400 hover:text-primary transition-all <?php if($page >= $total_pages) echo 'pointer-events-none opacity-30'; ?>"><span class="material-symbols-outlined text-sm">chevron_right</span></a>
                        <a href="<?php echo getUrl($total_pages, $search_query, $filter_cinema, $filter_date); ?>" class="px-2 py-1.5 rounded-lg bg-surface-dark border border-accent-dark text-slate-400 hover:text-primary transition-all <?php if($page >= $total_pages) echo 'pointer-events-none opacity-30'; ?>" title="Trang cuối"><span class="material-symbols-outlined text-sm">last_page</span></a>
                    </div>
                </div>
                <?php endif; ?>
            </div>
